...
class WebhookManager:
    def __init__(self, wallet_id: str):
        ...
        self.topic_handlers = {
            ...
            'oob_invitation': self.topic_oob_invitation,
    ...
    async def handle_topic(self, topic, payload):
        ...
        if not (handler := await self.topic_handlers.get(topic)):
    ...
    async def topic_oob_invitation(self, payload):
    ...
    async def topic_issue_credential_v2_0(self, payload):
        ...
        if payload.get('state') == 'offer-received':
            ...
            cred_offer = CredentialOffer(
                ...
                exchange_id=payload.get('cred_ex_id'),
                connection_id=payload.get('connection_id'),
                comment=payload.get('cred_offer').get('comment'),
                preview=preview,
            ).model_dump()
